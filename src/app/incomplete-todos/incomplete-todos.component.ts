...
export class IncompleteTodosComponent {
  ...
  ngOnInit() {
    this.httpClient.get<ApiResponse<Todo[]>>(BACKEND_URL + "/api/incomplete-todos", {
      withCredentials: true,
    }).subscribe( {
      next: (response)=> {
	this.incompleteTodos = response.mainBody;
	console.log(this.incompleteTodos);
	const ongoingTodo = this.incompleteTodos.find(todo => todo.status == "IN_PROGRESS" || todo.status == "PAUSED");
	if(ongoingTodo != undefined)
	  this.ongoingTodo = ongoingTodo;
      },
      error: (error)=> {
  ...
  startTodo(todo: Todo) {
    ...
    this.httpClient.put<ApiResponse<Todo>>(BACKEND_URL + "/api/start-todo", {}, {
      withCredentials: true,
      params: params,
    }).subscribe({
      next: (response) => {
